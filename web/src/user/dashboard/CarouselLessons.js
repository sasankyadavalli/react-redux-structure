...
class CarouselLessons extends Component {
  ...
  render() {
    ...
    <div>
     ...
     {lessonstList.map(lesson =>(
     
        <div className="carousel-lessons-img is-completed" data-class_id={lesson.class_id} onClick={this.goToClassPage} style={{backgroundImage: 'linear-gradient(rgba(4,29,65,0.10) 0%, #041E42 100%), url(' + lesson.lesson_image + ')'}}>
          <div className="carousel-lessons-title">1.1 {lesson.lesson_name}</div>
        </div>
      ))}
     </div>
    </div>
