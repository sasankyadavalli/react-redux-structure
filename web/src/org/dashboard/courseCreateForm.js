...
class CourseCreateForm extends Component {
    ...
    if (this.props.course_obj) {
      let arrayList=[];
        ...
        this.setState({
            ...
            'ribbons':arrayList,
            'course_type':this.props.course_obj.course_type
        });
  ...
  handleChange(e, validators = []) {
    ...
    if(e.target.name == "course_type"){
      this.setState({errors, 'course_type': (this.state.course_type == "yes") ? "no" : "yes"});
    }else{
  ...
  render() {
    ...
    <ModalBody>
      <Form onChange={this.checkTheFields}>
        ...
        <FormGroup>
          <Label check>
            <Input type="checkbox" name="course_type" checked={this.state.course_type === 'yes'} value={this.state.course_type} onChange={this.handleChange} />
