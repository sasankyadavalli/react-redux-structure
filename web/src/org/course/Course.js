...
class Course extends Component {
 ...
 componentDidMount() {
  ...
  handleChange(e, validators = []) {
    ...
    if(this.state.class_name && this.state.class_content && this.state.class_video_url && this.state.preview_image && (Object.keys(this.state.errors).length) == 0){
            ...
            if (response.status === 200) {
                ...
                return response.json().then(data => {
                    if (data) {
                        ...
                        if (this.state.class_name && this.state.class_content && this.state.class_video_url && this.state.preview_image) {
                            this.setState({ 'buttonDisable': false });
                        }
                    }
                });
  ...
  previewImageUnspalsh(url) {
     ...
     if (this.state.class_name && this.state.class_content && this.state.class_video_url && url) {
         this.setState({ 'buttonDisable': false });
     }
     this.toggle1();
 }
    ...
    <Modal isOpen={this.state.modal} toggle={this.toggle} className={this.props.className}>
        ...
        <ModalBody>
          ...
          <Container>
            ...
            <div className={url.id == this.state.ribbon_css_id ? "material-thumbnail is-selected" : "material-thumbnail"} id={url.id} data-ribbonid={url.ribbon_url} onClick={this.ribbonId}>
              {url.id == this.state.ribbon_css_id ? imagecheck : ""}
              <img id={url.id} src={url.ribbon_url} className="thumbnail"/>
            </div>
        ...
        <ModalFooter>
          ...
          <Button size="lg" color="cancel" onClick={this.toggle}>Cancel</Button>
        </ModalFooter>
    </Modal>
      ...
      <div className="course-page">
        ...
        <div className="course-page-body">
          <section className="course-body pl-6 pr-6" >
            <h6 className="mb-4">{this.state.classFormState == 'edit' ? 'Edit Draft Class' : 'Create New Class'}</h6>
            <Form onSubmit = {this.createClass}  id={this.state.class_id} onChange={this.checkTheFields}>
             <FormGroup color={errors.class_name ? "danger" : ""}>
              ...
              <Input type="text" name="class_name" value={this.state.class_name} onChange={(e) => this.handleChange(e, ['required'])} placeholder="Impact or Intrinsic" />
            ...
            <FormGroup color={errors.class_content ? "danger" : ""}>
              <Label for="class_content">Content</Label>
              <Input type="text" name="class_content" value={this.state.class_content} onChange={(e) => this.handleChange(e, ['required'])} placeholder="Write something" />
              {errors.class_content ? <FormFeedback>{errors.class_content}</FormFeedback> : "" }
            </FormGroup>
            <FormGroup color={errors.class_video_url ? "danger" : ""}>
              ...
              {errors.class_video_url ? <FormFeedback>{errors.class_video_url}</FormFeedback> : "" }
            </FormGroup>
            <FormGroup>
              ...
              <div className="r-1-wrapper">
                ...
                <Button outline color="primary" size="sm" onClick={this.changeUploadedRibbonBtn}>Change</Button>}
              </div>
            ...
            <Label>Preview Image</Label>
              <div className="mb-2">
                <Button outline color="primary" size="sm" onClick={this.toggle1}>Select stock image</Button>{' '}
                <span>
            ...
            <div className="mt-3 float-right">
                ...
                <Button size="sm" color="cancel" onClick={this.resetForm}>Cancel</Button>
            </div>
            </FormGroup>
            </Form>
          </section>
