...
class AddQuestion extends Component {
  constructor(props){
    super(props);
    this.state = {
      questions: [{'question':'','answer':'','id':1,'isShow':true,'options':[
...
viewquestion(e,obj){
  ...
  addQuestion(e){
    ...
    this.setState({
      questions: [...ques,{'question':'','answer':'','isShow':true,'id':ques.length+1,'options':[
      {'option':'','id':1,'correctanswer':true},
      {'option':'','id':2,'correctanswer':false},
      {'option':'','id':3,'correctanswer':false},
      {'option':'','id':4,'correctanswer':false}]}],
 ...
 render() {
  ...
  return (
      ...
      <Form onChange={this.checkTheFields}>
       {questions.map((question,i) =><FormGroup key={i}>
